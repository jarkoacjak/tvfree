...
def list_slovak_channels():
    """Zoznam slovenských staníc s opraveným logom Senzi."""
    # TV JOJ
    ...
    # SENZI TV - Opravené logo podľa tvojho odkazu
...
def play_video(stream_url, title):
    """Spustí video s hlavičkami pre simuláciu prehliadača Chrome."""
    ua = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36"
    
    # Pridanie hlavičiek (nutné pre JOJ stanice)
    headers = f"|User-Agent={urllib.parse.quote_plus(ua)}&Referer={urllib.parse.quote_plus('https://www.joj.sk/')}"
    final_url = stream_url + headers
    
    list_item = xbmcgui.ListItem(path=final_url)
    list_item.setInfo('video', {'title': title})
    
    # Aktivácia InputStream Adaptive pre m3u8 streamy
